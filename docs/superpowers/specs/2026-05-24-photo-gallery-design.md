...
- 水印配置面板（所有参数可实时调整并保存到 localStorage）:
  ...
  - 保存配置 / 恢复默认
  - 支持上传图片水印（Logo 等）
- 图片处理流程（健壮版，含 Worker + 断点续传）：
  1. 选择图片 → 显示在文件列表中
  2. 每个文件独立进入处理队列（并发控制：同时处理 2 张）
  3. **Web Worker 处理阶段**（不阻塞主线程）：
     - 用 `createImageBitmap` 高效解码图片（Worker 内）
     - `OffscreenCanvas` 绘制用户配置的水印（平铺/单点/居中）
     - `convertToBlob` 导出 JPEG（质量 0.95）
     - 处理完立即 `bitmap.close()` 释放解码内存
     - 进度实时回传主线程（处理进度 0-100%）
  4. **上传阶段**：
     - 使用 tus-js-client 分片上传（5MB/片）
     - 断网自动重试（指数退避）
     - 上传进度实时显示
     - 支持暂停/恢复
     - 完成后从队列移除
  5. Worker 接收文件 → 验证 → 写入 R2 → 返回公开 URL
- 上传进度条 + 结果反馈
...
最终决策：**R2 只存一份 20MB 带水印原图**，缩略图用 Cloudflare Images 动态生成。

| 项目 | 数量 | 单张大小 | 总量 |
|------|------|----------|------|
| 带水印原图（R2） | 300 张 | 20MB | **6GB** |
| 缩略图（Images 动态生成） | 300 张 | — | 0（不存 R2） |
| **R2 总占用** | — | — | **≈6GB** |

**结论**：6GB 在 R2 免费 10GB 额度内，空间充足。20MB 原图在本地电脑有备份。
